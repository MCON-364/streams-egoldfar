...
public class StreamHomework {
    ...
    public List<String> getTopCustomers(int n) {
        ...
        var customerTotals = getRevenueByCustomer();
        return customerTotals.keySet().stream()
                .sorted(Collections.reverseOrder(Comparator.comparing(customerTotals::get)))
                .limit(n)
                .toList();
    }
    ...
    public Map<String, Long> getCustomerOrderCounts() {
        ...
        return customerOrders.stream()
                .collect(Collectors.groupingBy(o -> o.customerId, Collectors.counting()));
    }
    ...
    public List<String> getCustomersWithMultipleOrders() {
        ...
        var customerOrderCount = getCustomerOrderCounts();
        return customerOrderCount.keySet().stream()
                .filter(c -> customerOrderCount.get(c) > 1)
                .toList();
    }
    ...
    public List<Product> getTopSellingProducts(int n) {
        ...
        var productToQuantity =  customerOrders.stream()
                .filter(o -> o.status == OrderStatus.DELIVERED)
                .flatMap(o -> o.items.stream())
                .collect(Collectors.groupingBy(
                        i -> i.product,
        ...
        return productToQuantity.keySet().stream()
                .sorted(Collections.reverseOrder(Comparator.comparingInt(productToQuantity::get)))
                .limit(n)
                .toList();
    }
    ...
    public Map<String, CategorySummary> getCategorySummary() {
        ...
        return customerOrders.stream()
                ...
                .collect(Collectors.groupingBy(
                     i -> i.product.category,
                        Collectors.teeing(
                                Collectors.summingDouble(i -> i.product.price * i.quantity),
                                Collectors.summingInt( i -> i.quantity),
                                CategorySummary::new
                        )
